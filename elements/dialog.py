...
def make_textbox(title, text, font_size=None, font_color=None, ok_text="Ok",
    ...
    if hline:
        ...
        if hline < 0:
            e_line = thorpy.Line.make(e_title.get_size()[0],"h")
        else:
            e_line = thorpy.Line.make(hline,"h")
        box = make_ok_box([e_title,e_line,e_text], ok_text=ok_text)
    return box
...
def launch_blocking_alert(title, text, parent=None, font_size=None, font_color=None,
                            ok_text="Ok", transp=True, alpha_dialog=150):
